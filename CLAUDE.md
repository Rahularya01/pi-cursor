...
Run `yarn check` (typecheck, lint, format:check, security-check, proto:check, vitest,
...
- Never hand-edit `src/proto/agent_pb.ts` — change `proto/agent.proto` and run `yarn proto:gen`.
...
- Node.js >= 22 is the only supported runtime. All Cursor HTTP/2 transport runs in-process via
  `node:http2`. Yarn 4 is the package manager — see AGENTS.md §7.
...
- No `node_modules/` is committed; run `yarn install` before typecheck/build/test if the working
  tree is fresh.
