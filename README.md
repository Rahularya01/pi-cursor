...
| `PI_CURSOR_NATIVE_SHELL`                       | Run a model's shell command inside the provider instead of handing it back to Pi's `bash` tool. **Default off**: Cursor's current server does not complete a turn from the streamed shell frames this build sends, so the turn parks on heartbeats. Set `1`/`true` once the schema is resynced.                                                                                                                      |
...
- **Every turn hangs after a line of preamble, on a current Pi:** Pi 0.86 moved the system prompt and tool declarations into the transcript's system messages, so a build before this fix sent `"You are a helpful assistant."` and no tools on every request — check `$TMPDIR/pi-cursor-lifecycle.jsonl` for a `request_size` line with `systemChars` near 28 and `toolCount: 0`. With nothing to call, the model reaches for Cursor's native shell, which then parks the turn. Update `pi-cursor`.
- **A shell command parks the turn:** a streamed shell is rejected back to Pi's `bash` tool by default because Cursor's server does not complete a turn from the shell frames this build sends. If `PI_CURSOR_NATIVE_SHELL=1` is set and turns hang on `exec_server` `shellStreamArgs` in the lifecycle log, unset it.
