...
- **`No API provider registered for api: cursor-native`:** This means the caller dispatched through Pi's compat `stream`/`streamSimple` registry and the `cursor-native` transport was not registered on _that same module instance_. On current builds the transport is registered during extension load; if it still fails, the usual cause is a stale nested peer: a `bun install`/`npm install` inside the package created `node_modules/@earendil-works/*` (a separate pi-ai instance with its own registry). Remove it (`rm -rf node_modules/@earendil-works`) or reinstall with the fixed package (peers are now optional + `bunfig.toml` `peer=false`) and restart Pi (or `/reload`). `/cursor.doctor` reports the condition as `staleNestedPeers`.
